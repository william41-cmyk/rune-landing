...
import { useState, useCallback, useMemo, useEffect, useRef, ReactNode, Suspense } from "react";
...
type SearchEntry = { slug: string; pageTitle: string; section: string; matchText: string; type: "title" | "subtitle" | "heading" };
const searchIndex: SearchEntry[] = [];
for (const [slug, data] of Object.entries(pageData)) {
  const base = { slug, pageTitle: data.title, section: data.section };
  searchIndex.push({ ...base, matchText: data.title, type: "title" });
  if (data.subtitle) searchIndex.push({ ...base, matchText: data.subtitle, type: "subtitle" });
  for (const t of data.toc) searchIndex.push({ ...base, matchText: t.label, type: "heading" });
}

function SidebarSectionComponent({
  section, activeSlug, onSelect,
}: {
...
function DocsPageInner() {
  ...
  // Search results — only when ≥3 chars (full word gate)
  const searchResults = useMemo(() => {
    ...
    for (const entry of searchIndex) {
      if (entry.matchText.toLowerCase().includes(q) && !seen.has(entry.slug)) {
        seen.add(entry.slug);
        results.push(entry);
    ...
    <Box minH="100vh" bg="rune.bg" overflowX="hidden">
      <Box position="fixed" top={0} left={0} right={0} w="100%" zIndex={1000}
        ...
        <Flex maxW="1400px" mx="auto" px={{ base: 4, md: 6 }} py={3}
          ...
          <Box ref={searchRef} flex={1} maxW="400px" mx={6} display={{ base: "none", md: "block" }} position="relative">
            <InputGroup size="sm">
              ...
              <Input placeholder="Search docs..." bg="rune.overlay.soft"
                ...
                _focus={{ borderColor: "rune.border.default", boxShadow: "none", bg: "rune.overlay.hover", outline: "none" }}
            ...
            {searchFocused && searchResults.length > 0 && (
              <Box position="absolute" top="calc(100% + 6px)" left={0} right={0}
                bg="rune.surface" border="1px solid" borderColor="rune.border.subtle"
                borderRadius="12px" py={1.5} zIndex={2000}
                ...
                {searchResults.map((r) => (
                  <Flex key={r.slug} align="center" gap={2} px={3} py={2} mx={1.5}
                    ...
                    <Icon as={FiSearch} boxSize={3} color="rune.text.muted" flexShrink={0} />
                    <Box flex={1} minW={0}>
                      <Text fontSize="sm" fontWeight={500} color="rune.text.primary"
                        letterSpacing="-0.03em" noOfLines={1}>
                        {r.pageTitle}
                      </Text>
                      <Text fontSize="xs" color="rune.text.muted" letterSpacing="-0.03em" noOfLines={1}>
                        {r.section}{r.type === "heading" ? ` → ${r.matchText}` : ""}
                      </Text>
                    </Box>
                  </Flex>
                ))}
          ...
          <HStack spacing={4}>
            <ChakraLink href="/" fontSize="sm" fontWeight={500} color="rune.text.muted"
              _hover={{ color: "rune.text.primary", textDecoration: "none" }}
              transition="all 0.2s" display={{ base: "none", md: "flex" }}>
              Home
            </ChakraLink>
            <Link href="/download" passHref>
              <Button size="sm" bg="rune.btn.primary.bg" color="rune.btn.primary.text" fontWeight={500}
                fontSize="sm" borderRadius="full" px={4}
                _hover={{ opacity: 0.85 }} _active={{ opacity: 0.75 }}
                transition="all 0.2s">
                Download
              </Button>
            </Link>
      ...
      <Flex maxW="1400px" mx="auto" pt="60px" minH="100vh">
        <Flex as="aside" direction="column" w="260px" flexShrink={0} position="fixed"
          top="60px" bottom={0}
          ...
          <VStack align="stretch" spacing={2}>
            {sidebarData.map((section) => (
              <SidebarSectionComponent key={section.title} section={section}
                activeSlug={activeSlug} onSelect={handleSelect} />
            ))}
          </VStack>
        </Flex>
        {sidebarOpen && (
              ...
              <Flex borderTop="1px solid" borderColor="rune.border.subtle"
                ...
                {currentPage.prev ? (
                  <Flex align="center" gap={2} cursor="pointer" color="rune.text.muted"
                    transition="color 0.2s" _hover={{ color: "rune.text.primary" }}
